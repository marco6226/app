...
export class StorageService {
    ...
    init() {
        ...
        if (controller != null) {
            controller.then((db: SQLiteObject) => {
                this.database = db;
    ...
    guardarInspeccion(inspeccion: Inspeccion): any {
        return new Promise((resolve, reject) => {
            ...
            this.database.executeSql(sql, [inspeccion['hash'], JSON.stringify(inspeccion), this.getEmpresa().id])
                .then(() => resolve())
    ...
    getInspecciones(): Promise<Response<Inspeccion[]>> {
        return new Promise((resolve, reject) => {
            ...
            this.database.executeSql(sql, [this.getEmpresa().id])
                .then(resultset => {
    ...
    actualizarInspeccionPendiente(inspeccion: Inspeccion): any {
        return new Promise((resolve, reject) => {
            ...
            this.database.executeSql(sql, [JSON.stringify(inspeccion), inspeccion['hash']])
                .then(() => resolve())
                .catch(err => reject(err));
        });
    }
    ...
    getListaInspeccion(idLista: string, versionLista: number): any {
        return new Promise((resolve, reject) => {
            ...
            this.database.executeSql(sql, [idLista, versionLista, this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                })
                .catch(err => reject(err));
    ...
    getListasInspeccion() {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM inp_lista_inspeccion_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                })
                .catch(err => reject(err));
    ...
    getSistemaCausaInmediata() {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM sec_sistema_causa_inmediata_v2 WHERE empresa_id = ? LIMIT 1", [this.getEmpresa().id])
                .then(resultset => {
    ...
    getDesviaciones(): Promise<Response<Desviacion[]>> {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM sec_desviacion_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                })
                .catch(err => reject(err));
    ...
    getDesviacionesFav(): Promise<Response<Desviacion[]>> {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM sec_desviacion_fav_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                })
                .catch(err => reject(err));
    ...
    getActasCopasst() {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM cop_acta_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                })
                .catch(err => reject(err));
    ...
    guardarSyncActaCopasst(acta: Acta) {
        return new Promise((resolve, reject) => {
            ...
            this.database.executeSql(sql, [acta['hashId'], JSON.stringify(acta), this.getEmpresa().id])
                .then(() => resolve());
        });
    }
    ...
    getSyncActasCopasst() {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM cop_acta_sync_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                });
        });
    ...
    borrarSyncActasCopasst(acta: Acta) {
        return new Promise((resolve, reject) => {
            this.database.executeSql("DELETE FROM cop_acta_sync_v2 WHERE hashId = ?", [acta['hashId']])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                });
        });
    ...
    guardarSyncObservacion(obser: Observacion) {
        return new Promise((resolve, reject) => {
            ...
            this.database.executeSql(sql, [obser['hashId'], JSON.stringify(obser), this.getEmpresa().id])
                .then(() => resolve());
        });
    }
    ...
    getSyncObservaciones() {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM auc_observacion_sync_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                });
        });
    ...
    borrarSyncObservacion(obser: Observacion) {
        return new Promise((resolve, reject) => {
            this.database.executeSql("DELETE FROM auc_observacion_sync_v2 WHERE hashId = ?", [obser['hashId']])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                });
        });
    ...
    getSistemaCausaRaiz() {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM sec_sistema_causa_raiz_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset)['data'][0]);
                })
                .catch(err => reject(err));
    ...
    getAreas() {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM emp_area_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                })
                .catch(err => reject(err));;
    ...
    getTarjetas() {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM auc_tarjeta_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
    ...
    getSistemaNivelRiesgo() {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM sec_sistema_nivel_riesgo_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                })
                .catch(err => reject(err));;
    ...
    getProgramaciones() {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM inp_programacion_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                })
                .catch(err => reject(err));;
    ...
    updateProgramacion(programacion: Programacion) {
        return new Promise((resolve, reject) => {
            this.database.executeSql("UPDATE inp_programacion_v2 SET body = ? WHERE id = ?", [JSON.stringify(programacion), programacion.id])
                .then(() => {
                    resolve();
                });
        });
    ...
    getSistemaCausaAdministrativa() {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM sec_sistema_causa_administrativa_v2 WHERE empresa_id = ? LIMIT 1", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset)['data'][0]);
                })
                .catch(err => reject(err));
    ...
    guardarAnalisisDesviacion(analisis: AnalisisDesviacion): any {
        return new Promise((resolve, reject) => {
            this.getAnalisisDesviacion(analisis.id)
                .then(resp => {
                    if (resp.count > 0) {
                        let sql = "UPDATE sec_analisis_desviacion_v2 SET body = ? WHERE id = ?";
                        this.database.executeSql(sql, [JSON.stringify(analisis), analisis.id])
                            .then(() => resolve())
                            .catch(err => {
                                reject(err)
                            });
                        ...
                        this.database.executeSql(sql, [analisis.id, JSON.stringify(analisis), this.getEmpresa().id])
                            .then(() => resolve())
                            .catch(err => reject(err));
                    }
                });
    ...
    guardarAnalisisSync(analisis: AnalisisDesviacion): any {
        return new Promise((resolve, reject) => {
            this.getAnalisisSync(analisis['hashId'])
                .then(resp => {
                    if (resp.count > 0) {
                        let sql = "UPDATE sec_analisis_desv_sync_v2 SET body = ? WHERE hashId = ?";
                        this.database.executeSql(sql, [JSON.stringify(analisis), analisis['hash']])
                            .then(() => resolve())
                            .catch(err => reject(err));
                    } else {
                        let sql = "INSERT INTO sec_analisis_desv_sync_v2(hashid, body, empresa_id) VALUES (?, ?, ?)";
                        this.database.executeSql(sql, [analisis['hash'], JSON.stringify(analisis), this.getEmpresa().id])
                            .then(() => resolve())
                            .catch(err => reject(err));
                    }
                });
    ...
    getAnalisisSync(hashId: string): Promise<Response<AnalisisDesviacion[]>> {
        return new Promise((resolve, reject) => {
            ...
            this.database.executeSql(sql, [hashId, this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                })
                .catch(err => reject(err));
    ...
    getManualesUsuario(): any {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT body FROM conf_manual_usuario_v2 WHERE empresa_id = ?", [this.getEmpresa().id])
                .then(resultset => {
                    resolve(this.toResolveObject(resultset));
                })
                .catch(err => reject(err));
